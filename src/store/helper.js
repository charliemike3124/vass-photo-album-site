...
const GettersHelper = {
  ...
  mapKeys(prop, targetKey) {
    ...
    return (state) => (vals) =>
      filter(state)(vals).sort((a, b) => vals.indexOf(a[targetKey]) - vals.indexOf(b[targetKey]));
  },
};
...
const MutationsHelper = {
  ...
  extendRecordInList:
    ...
    (state, data) => {
      ...
      return index < 0 ? state[key].push(val) : state[key].splice(index, 1, Object.assign({}, state[key][index], val));
    },
